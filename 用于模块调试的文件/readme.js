...
function 完美校园打卡(){
    ...
    while(true){
        //错误处理
        if(text("我知道了").exists()){
            text("我知道了").findOne(30000).click();
        }
        if(text("姓名").exists() && text("今日体温").exists()){
            log("已加载完打卡页面");
            break;
        }
    }
    ...
    if (text("返回编辑").findOne(20000)) {
        ...
        if (myapp.confirmX == 0 && myapp.confirmY == 0){
            sleep(1500);
            //获取确认提交坐标
            MyThreshold = GetConfirm(MyThreshold);
        }
        ...
        while(true){
            ...
            if(text("打卡成功").findOne(1000)!= null){
                alert("欢迎回来，已经完成打卡完成啦！\n(￣y▽￣)╭ Ohohoho.....");
                sleep(300);
                关闭应用(myapp.packageName);
                return 0;
            }else{
                if(text("返回编辑").exists() && MyThreshold >= 10)
                {
                        MyThreshold -= 5;
                        MyThreshold = GetConfirm(MyThreshold);
                }else{
...
//获取确认提交坐标
function GetConfirm(nThreshold) {
    //请求截图
    ...
    while (nThreshold >= 10) {
        ...
        if(point){
            ...
            SaveData();
            Log("找到啦:" +  myapp.confirmX + myapp.confirmY);
            break;
        }else{
            toastLog("没找到,继续尝试");
            nThreshold -= 5;
